...
const ssl = process.env.BCIS_DB_SSL === 'disable' ? false : {rejectUnauthorized:false};
const clientConfig = { connectionString: db, ssl };
...
const one = async (rel, client=null) => {
  const own = !client; const c = client ?? new Client(clientConfig);
...
const main = async () => {
  const guard = new Client(clientConfig); await guard.connect();
  try {
  ...
  const validate = new Client(clientConfig); await validate.connect();
  try {
  ...
  const attempts = await Promise.all(Array.from({length:25}, async (_,i) => {
    const c = new Client(clientConfig); await c.connect();
  ...
  const rls = new Client(clientConfig); await rls.connect();
  try {
